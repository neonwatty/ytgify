// WXT-migrated YouTubeGifMaker class
...
import { WXTYouTubeNavigator, YouTubeNavigationEvent } from './navigation';
import { ShadowDOMUIManager } from './ui-manager';
import { injectionManager } from '@/src/content/injection-manager';
import { extensionStateManager } from '@/src/shared';
import { youTubeAPI, YouTubeAPIIntegration } from '@/src/content/youtube-api-integration';
import { ContentScriptFrameExtractor, ContentFrameExtractionRequest } from '@/src/content/frame-extractor';
import { gifProcessor } from '@/src/content/gif-processor';
import { playerIntegration } from '@/src/content/player-integration';
import { playerController } from '@/src/content/player-controller';
import { TimelineOverlayWrapper } from '@/src/content/timeline-overlay-wrapper';
import { TimelineOverlayWizard } from '@/src/content/timeline-overlay-wizard';
import { overlayStateManager } from '@/src/content/overlay-state';
import { cleanupManager } from '@/src/content/cleanup-manager';
import { initializeContentScriptFrameExtraction } from '@/src/content/frame-extractor';
import { themeDetector, youtubeMatcher } from '@/src/themes';
import { ResolutionScaler } from '@/src/processing/resolution-scaler';
import { parseResolution } from '@/src/utils/resolution-parser';
import { engagementTracker } from '@/src/shared/engagement-tracker';
import NewsletterWizard from '@/src/content/newsletter-wizard/NewsletterWizard';

export class YouTubeGifMaker {
  private ctx: ContentScriptContext;
  private navigator: WXTYouTubeNavigator;
  ...
  constructor(ctx: ContentScriptContext, navigator: WXTYouTubeNavigator, uiManager: ShadowDOMUIManager) {
    this.ctx = ctx;
  ...
  // CSS is handled by WXT shadow DOM - no manual injection needed
  ...
  private setupNavigationListener() {
    this.navigationUnsubscribe = this.navigator.onNavigation(
      async (event: YouTubeNavigationEvent) => {
        this.log('info', '[Content] YouTube navigation detected', {
          ...
          canCreateGif: this.navigator.canCreateGif(),
        });
        ...
        if (this.isActive && !this.navigator.canCreateGif()) {
          this.log(
            'info',
  ...
  private async findVideoElement() {
    ...
    this.videoElement = await this.navigator.waitForVideoElement(10000);
    ...
    if (this.videoElement) {
      ...
      if (!this.gifButton && this.navigator.canCreateGif()) {
        const injected = playerIntegration.injectButton((event) => {
          event.preventDefault();
          this.handleGifButtonClick();
        });
      ...
      const currentState = this.navigator.getCurrentState();
  ...
  private async handleGifButtonClick() {
    ...
    const canCreate = this.navigator.canCreateGif();
  ...
  private showWizardOverlay(message: ShowTimelineRequest) {
    try {
      // CSS is handled by WXT shadow DOM
  ...
  private detectCompactMode(): boolean {
    ...
    const player = this.navigator.getPlayerContainer();
    if (player) {
  ...
  private getCurrentVideoState() {
    ...
    if (!this.videoElement) {
      ...
      this.videoElement = this.navigator.getVideoElement();
      if (!this.videoElement) {
