...
const FileSizePreview: React.FC<FileSizePreviewProps> = ({ gifBlob, settings, className }) => {
  ...
  React.useEffect(() => {
    if (settings) {
      ...
      const duration =
        settings.endTime && settings.startTime ? settings.endTime - settings.startTime : 3;
        ...
        {currentSize ? (
          <>
            <div className="flex items-center justify-between">
              <span className="text-sm">{actualSize ? 'Current Size:' : 'Estimated Size:'}</span>
            ...
            {sizeInfo && (
              <div className="text-xs text-muted-foreground">
                ...
                {sizeInfo.status === 'large' &&
                  '⚠ Large file, consider reducing quality or duration'}
              </div>
            )}
...
export const ExportPanel: React.FC<ExportPanelProps> = ({
  ...
  React.useEffect(() => {
    const checkClipboardSupport = async () => {
      ...
      setClipboardSupported(
        Boolean(capabilities?.supportsNativeAPI && capabilities?.supportsImages)
      );
    };
  ...
  }, [canExport, onExport, title, description, tags, exportOptions, filename, gifBlob]);
          ...
          >
            {exportProgress ? (
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <label className="flex items-center gap-2">
                      ...
                      />
                      <span
                        className={cn('text-sm', !clipboardSupported && 'text-muted-foreground')}
                      >
                        📋 Copy to Clipboard
                      </span>
                      {!clipboardSupported && (
                ...
                {/* File Size Preview */}
                <FileSizePreview gifBlob={gifBlob} settings={settings} />
              </div>
            )}

            {/* Export and Share Buttons */}
            <div className="pt-6 border-t space-y-3">
              <Button onClick={handleExport} disabled={!canExport} className="w-full" size="lg">
                {isExporting ? (
                  <>
...
export const ExportPanelDemo: React.FC<ExportPanelDemoProps> = ({
  ...
  const handleExport = React.useCallback(
    async (metadata: Partial<GifData>, options: ExportOptions) => {
