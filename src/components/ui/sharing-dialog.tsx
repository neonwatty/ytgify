  ...
  return (
    ...
    >
      ...
      <div className="space-y-1">
        ...
        <div className="text-xs text-muted-foreground leading-tight">{target.description}</div>
        {!target.supported && <div className="text-xs text-destructive">Not supported</div>}
      </div>
      {(loading || isSharing) && (
      ...
      <div className="grid grid-cols-3 gap-2">
        {socialPlatforms.map((platform) => (
          <Button
            ...
            className={cn('h-auto p-3 flex flex-col gap-1 text-center', platform.color)}
          >
        ...
        if (result.success) {
          if (result.shared) {
            setShareStatus({
              success: true,
              message:
                targetId === 'url-copy' ? 'Link copied to clipboard!' : 'Shared successfully!',
            });
          ...
          {shareUrl && (
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                />
                <Button variant="outline" size="sm" onClick={handleCopyUrl} disabled={isSharing}>
                  📋 Copy
                </Button>
              </div>
            </div>
