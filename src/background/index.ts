...
chrome.runtime.onInstalled.addListener(sharedErrorHandler.wrapWithErrorBoundary(async (details) => {
  ...
  try {
    sharedLogger.info('[Background] YouTube GIF Maker extension installed', { 
...
chrome.runtime.onStartup.addListener(sharedErrorHandler.wrapWithErrorBoundary(async () => {
  sharedLogger.info('[Background] YouTube GIF Maker extension started', {}, 'background');
...
chrome.runtime.onMessage.addListener((
  ...
  try {
    ...
    if (!message || !message.type) {
      sharedLogger.warn('[Background] Invalid message received', { 
        message, 
    ...
    sharedLogger.debug('[Background] Received message', { 
      type: messageType,
    ...
    return sharedErrorHandler.withRecovery(
      () => messageHandler.handleMessage(message, sender, sendResponse),
      {
        maxRetries: 1,
        delayMs: 100,
        fallbackAction: async () => {
          sharedLogger.warn('[Background] Using message fallback handler', {
            messageType,
            messageId
          }, 'background');
      ...
      .catch((error) => {
        ...
        sharedLogger.error('[Background] Message handling failed', { 
          error: error instanceof Error ? error.message : String(error), 
          messageType,
          messageId 
        }, 'background');
    ...
    sharedLogger.error('[Background] Critical message handling error', { 
      error: error instanceof Error ? error.message : String(error) 
    }, 'background');
...
chrome.runtime.onInstalled.addListener((details) => {
  logger.info('[Background] YouTube GIF Maker extension installed', { 
    reason: details.reason,
    version: chrome.runtime.getManifest().version
  });
});
...
async function initializeStorage(): Promise<void> {
  ...
  try {
    ...
    endTimer();
    sharedLogger.error('[Background] Failed to initialize storage', { 
      error: error instanceof Error ? error.message : String(error) 
    }, 'background');
...
async function migratePreferences(preferences: Record<string, unknown>): Promise<void> {
  try {
    ...
    if (needsUpdate) {
      ...
      sharedLogger.info('[Background] Migrated user preferences', { 
    ...
    sharedLogger.warn('[Background] Failed to migrate preferences', { 
      error: error instanceof Error ? error.message : String(error) 
    }, 'background');
  }
}
