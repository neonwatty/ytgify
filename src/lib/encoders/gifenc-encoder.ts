...
export class GifencEncoder extends AbstractEncoder {
  ...
  private async performEncoding(
    ...
    const _format = this.createGifFormat(options);
  ...
  private createGifFormat(_options: EncodingOptions) {
    return {
      width: _options.width,
      height: _options.height,
      loops: _options.loop ? 0 : 1,
      colorResolution: 8,
    };
  }

  private createGifHeader(_options: EncodingOptions): Uint8Array {
  ...
  private generateRecommendations(
    ...
    if (options.quality === 'high' && encodingTime > 10000) {
      recommendations.push(
        'High quality setting with long encoding time, consider "medium" quality'
      );
    }
